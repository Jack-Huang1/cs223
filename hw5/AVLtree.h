...
void treeDestroyAVL(struct tree **root);

/* insert an element into a tree pointed to by root */
void treeInsertAVL(struct tree **root, char * newElement);
...
int treeHeightAVL(const struct tree *root);

/* return size of tree */
size_t treeSizeAVL(const struct tree *root);

/* pretty-print the contents of a tree */
void treePrint(const struct tree *root, int which);
